...
string ThrillerScene::get_handler_id() const {
return handler_id;
}
...
bool ThrillerScene::init() {
  ...
  if (font == nullptr) {
    ...
    bg->init(100, 100);
    bg->load("bg.jpeg");
    text_display = std::make_shared<KaraokeTextDisplay>("The quick brown fox jumps over the lazy dog", textColor, font, renderer);
  }
